"""Top companies for loan rows from combined RM WPR and RM WPRC dataframes."""
...
class RmWaitingLoanTopCompaniesPaths:
    """Lakehouse relative path used by this derived output."""

    output: str = "RMWaitingLoanTopCompanies"
...
def build_rm_waiting_loan_top_companies(
    *,
    rm_wpr_df: DataFrame,
    rm_wprc_df: DataFrame,
    limit: int = 10,
) -> DataFrame:
    """Combine RM WPR and RM WPRC dataframes, then return top loan companies."""
    ...
    grouped = filtered.groupBy(company_col).agg(
        F.count(F.lit(1)).cast("bigint").alias("Nombre")
    )
    return grouped.orderBy(F.col("Nombre").desc()).limit(limit)


def build_rm_waiting_loan_top_companies_from_lakehouse(
    *,
    lakehouse_name: str,
    ...
    """Build RM WPR and RM WPRC, then return top loan companies."""
    ...
    return build_rm_waiting_loan_top_companies(
        rm_wpr_df=rm_wpr_df,
...
def run_rm_waiting_loan_top_companies_pipeline(
    *,
    lakehouse_name: str,
    rme_paths: RmePaths = RmePaths(),
    top_paths: RmWaitingLoanTopCompaniesPaths = RmWaitingLoanTopCompaniesPaths(),
    mode: str = "overwrite",
    spark: SparkSession | None = None,
    today: date | None = None,
    limit: int = 10,
) -> DataFrame:
    """Build and write the derived loan top-companies table, then return it."""

    output_df = build_rm_waiting_loan_top_companies_from_lakehouse(
        lakehouse_name=lakehouse_name,
        rme_paths=rme_paths,
        spark=spark,
        today=today,
...
__all__ = [
    "RmWaitingLoanTopCompaniesPaths",
    "build_rm_waiting_loan_top_companies",
    "build_rm_waiting_loan_top_companies_from_lakehouse",
    "run_rm_waiting_loan_top_companies_pipeline",
]
